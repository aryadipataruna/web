...
<body class="bg-gray-100">
    ...
    <script>
        // ========== 1. STATE & DATA (Data contoh, ganti dengan data dari backend Anda) ==========
        const state = {
            cartItems: [
                {
                    id: "P001",
                    name: "ASUS ROG ZEPHYRUS G16 GU603VI RTX4070",
                    description: "i7 13620H, 32GB RAM, 1TB SSD, W11, 16.0\" FHD 165Hz",
                    notes: "NON BUNDLING, 16GB 1TBSSD",
                    price: 24479000,
                    originalPrice: 24499000,
                    quantity: 1,
                    checked: false,
                    store: "Top Tech Comp",
                    image: "https://placehold.co/100x100/E0E0E0/757575?text=Laptop+Gaming"
                },
                {
                    id: "P002",
                    name: "Apple iPhone 11 | 11 Pro | 11 Pro Max",
                    description: "512GB 256GB 64GB Second iBox",
                    notes: "iPhone 11, Face ID 64gb",
                    price: 3880000,
                    originalPrice: null,
                    quantity: 1,
                    checked: false,
                    store: "GudangGadget",
                    image: "https://placehold.co/100x100/E0E0E0/757575?text=iPhone+11"
                }
            ],
            stores: [
                { name: "Top Tech Comp", logo: "https://placehold.co/24x24/28A745/FFFFFF?text=S" },
                { name: "GudangGadget", logo: "https://placehold.co/24x24/4A90E2/FFFFFF?text=G" }
            ],
            userAvailablePoints: 5000,
            addresses: [
                {
                    id: "ADDR001",
                    label: "Kos",
                    ...
                    phone: "081227803688",
                    fullAddress: "Kost Swargaloka 19A, Caturtunggal, Depok, Kab. Sleman, D.I. Yogyakarta, 55281",
                    isPinpointed: true,
                    isPrimary: true
                }
            ],
            selectedAddressId: "ADDR001",
            checkout: {
                totalShippingCost: 0,
                ...
                paymentTimeout: null,
                // Variabel untuk menyimpan ID pesanan/nota setelah berhasil
                currentOrderId: null
            }
        };
        ...
        function renderCartListDOM() {
            ...
            if (state.cartItems.length === 0) {
                container.innerHTML = `<div class="text-center py-8 text-gray-500 empty-cart-message">
                    ...
                    <a href="#" class="text-green-600 hover:underline mt-2 inline-block">Mulai belanja</a>
                </div>`;
            ...
            let html = `
                <div class="flex justify-between items-center mb-6 pb-4 border-b border-gray-200">
                    ...
                    <button class="text-red-500 hover:text-red-700 font-medium" id="removeSelectedBtn">
                ...
                items.forEach(item => {
                    ...
                    html += `
                        <div class="flex flex-col sm:flex-row items-start sm:items-center py-4 border-t border-gray-200 product-item" data-product-id="${item.id}">
                            ...
                            <img src="${item.image}" alt="Gambar ${item.name}" class="w-24 h-24 object-cover rounded-lg mx-4">
                            <div class="flex-grow">
                            ...
                            <div class="flex items-center space-x-3 mt-3 sm:mt-0 self-center sm:self-auto">
                                <button class="text-gray-500 hover:text-red-500 action-btn" data-action="wishlist" data-id="${item.id}"><i class="far fa-heart"></i></button>
                                <button class="text-gray-500 hover:text-red-500 action-btn" data-action="remove-item" data-id="${item.id}"><i class="far fa-trash-alt"></i></button>
                                <div class="flex items-center border border-gray-300 rounded-md">
                                    <button class="px-2 py-1 text-gray-700 hover:bg-gray-200 rounded-l-md quantity-btn" data-action="decrease" data-id="${item.id}">-</button>
            ...
            document.getElementById('cartIconCount').textContent = state.cartItems.length;
            bindCartEvents();
        }
        ...
        function renderCartSummaryDOM() {
            ...
            if (selectedItems.length > 0) {
                initialShippingCost = subtotal < 1500000 ? 100000 : 0;
            }
            ...
            `;
            document.getElementById('proceedToCheckoutButton')?.addEventListener('click', handleProceedToCheckout);
        }
                    ...
                    itemsHTML += `
                        <div class="flex items-center justify-between py-2">
                            <div class="flex items-center">
                                <img src="${item.image}" alt="Gambar ${item.name}" class="w-12 h-12 object-cover rounded-md mr-3">
                                <div>
        ...
        function renderCheckoutSummaryDOM() {
            ...
            if (selectedCartItems.length > 0) {
                totalCheckoutShipping = subtotal < 1500000 ? 100000 : 0;
            }
            ...
            const pointsRedeemedValue = state.checkout.redeemedPointsAmount * 10000; // Asumsi 1 poin = Rp 10.000 (sesuaikan jika perlu)
            const discount = 0; // Diskon barang lain jika ada
            ...
            container.innerHTML = `
                ...
                <button id="payNowButton" class="w-full bg-green-500 text-white py-3 rounded-lg mt-6 font-semibold hover:bg-green-600 transition duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50">
                    Bayar Sekarang
                </button>
            `;
        ...
        function handleProceedToCheckout() {
            ...
            state.checkout.currentOrderId = null; // Reset order ID
        ...
        function startPaymentProcess() {
            ...
            // yang akan mengembalikan ID pesanan (no_nota)
            state.checkout.currentOrderId = "NOTA-" + new Date().getTime(); // Contoh ID Pesanan Dinamis
            ...
            if (timerDisplay) {
                ...
                let timeLeft = 15; // Simulasi waktu bayar
                timerDisplay.textContent = `Sisa Waktu Pembayaran: ${timeLeft} detik`;
        ...
        function handlePaymentTimeout() {
            ...
            renderCheckoutSummaryDOM();
            console.log("Barang simulasi tersedia kembali.");
            state.checkout.currentOrderId = null; // Reset order ID
        }
        ...
        function handleConfirmPaymentProof() {
            ...
            const isProofValid = true; // Simulasi validasi bukti
            ...
            if (isProofValid) {
                ...
                // === MODIFIKASI UNTUK REDIRECT ===
                // Pastikan state.checkout.currentOrderId sudah diisi dengan ID pesanan/nota yang valid
                // dari backend setelah proses 'Bayar Sekarang' (atau proses pembuatan pesanan).
                const orderIdForNota = state.checkout.currentOrderId; // Pastikan ini berisi ID Nota yang benar

                    if (orderIdForNota) {
                        console.log('Pembayaran berhasil, memuat PDF nota untuk pesanan: ' + orderIdForNota + ' di tab ini.');

                        // Mengganti window.open dengan window.location.href untuk membuka di tab yang sama
                ...
                // Logika di bawah ini mungkin tidak akan berjalan jika redirect terjadi sangat cepat.
                // Untuk simulasi, kita tetap biarkan.
                ...
                state.cartItems = state.cartItems.filter(item => !item.checked);
                // renderCartListDOM(); // Tidak perlu jika sudah redirect
                // renderCartSummaryDOM(); // Tidak perlu jika sudah redirect
                // renderCheckoutSummaryDOM(); // Tidak perlu jika sudah redirect
            } else {
                handlePaymentTimeout();
            }
        }
        ...
        function bindCartEvents() {
            ...
            cartContainer.addEventListener('click', (e) => {
                ...
                if (targetButton.id === 'removeSelectedBtn') {
                    ...
                    handleRemoveItem(targetButton.dataset.id);
